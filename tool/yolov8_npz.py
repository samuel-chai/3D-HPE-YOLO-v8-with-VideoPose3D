...
def save_keypoints_as_npz(result, save_path):
    np.savez(save_path, keypoints=result)
...
if __name__ == '__main__':
    ...
    # 弹出文件保存对话框选择保存关键点的输出位置
    output_keypoints_path = filedialog.asksaveasfilename(title="选择保存关键点文件位置", defaultextension=".npz", filetypes=[("NPZ files", "*.npz")])
    if not output_keypoints_path:
    ...
    with tqdm(total=total_frames, desc="Processing frames", unit="frame", ncols=100) as pbar:
        while True:
            ...
            if keypoints is not None:
                filtered_keypoints = keypoints[:, :3]  # 提取 x, y, conf
    ...
    save_keypoints_as_npz(keypoints_array, output_keypoints_path)
